...
@section('title', Lang::get('content.character search'))
@section('sub_title', '')
...
<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
	<div class="row m-b conteudo">
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 p-b p-t">
			{{ Form::open(array('action' => 'PlayersController@getBusca', 'class' => 'form', 'name' => 'filtro_busca', 'method' => 'get')) }}
			...
			<div class="form-group">
				{{ Form::label('status', Lang::get('content.status')) }}
				{{ Form::select('status', $status = array_add($status, -1, Lang::choice('content.all', 1)), -1, ['class' => 'form-control']) }}
			</div>
...
<div class="col-lg-7 col-md-7 col-sm-12 col-xs-12 conteudo m-b">
	<div class="row m-b">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 m-b p-b p-t titulo">
			<h3 class="foNW foNS m-z p-z hidden-sm hidden-xs">{{ Lang::get('content.characters found in the') }} {{ $configuracoes->title or 'App Name' }}</h3>
			<h5 class="m-z p-z hidden-lg hidden-md">{{ Lang::get('content.characters found in the') }} {{ $configuracoes->title or 'App Name' }}</h5>
		</div>
				...
				<table class="table display table-hover responsive nowrap" width="100%" name="dataTables-example">
					...
					<tbody>
						...
						<tr>
							...
							@endif
							<td>{{ $personagem->skill_fishing or '0' }}</td>
...
<script type="text/javascript" async="async">
	$(document).ready(function(){
		$('form[name=filtro_busca] input[name=name]').val('{{ (Input::has("name")) ? Input::get("name") : "" }}');
